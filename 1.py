...
# Please don't use this to disturb anyone.


#If it doesn't work for you, or if you have any recommendations, dm me on Instagram @lukka.lekic
...
def fiokokBeolvas():
  ...
  with open("list_of_accounts.txt", "r") as olvas:
...
def ellenorzes():
  ...
  for o in range(fiokokszama):
    ...
    if valasz.status_code == 200:
      print("Working... -> {} ".format(fiokok[o]))
    else:
      hibak = hibak + 1
      hibas.append(fiokok[o])
      print("Please check the user you inserted -> {} ".format(fiokok[o]))
  clear()
  ...
  if hibak > 0:
    print("There is  {} accounts/users that does not exist.".format(hibak))
    print("The accounts are:")
    for g in range (len(hibas)):
      print(hibas[g])
    valasztas = ""
    while valasztas not in ["Y", "N"]:
        valasztas = input("The accounts/users shown do not exist. Would you like to remove them? (Y/N): ")
        valasztas = valasztas.upper()
        if valasztas == "Y":
          for g in range (len(hibas)):
            fiokok.remove(hibas[g])
            with open("list_of_accounts.txt", "w") as iras:
              iras.write('\n'.join(fiokok))
          ...
          print("The answer is N (NO), and there is a possibility that the program may return a 404 error.")
          fiokokStringx = ','.join(fiokok)
          eredmenyx = hashlib.md5(fiokokStringx.encode())
          with open("MD5.md5", mode='r+') as md5:
            md5.seek(0) 
...
def ellenorzesMD():
  ...
  if eredmeny.hexdigest() == "346c0131861b4b0811559318a7954187":
    print("The file list_of_accounts.txt only contains example accounts. Please add the actual accounts.")
    ...
    with open("MD5.md5", mode='r+') as md5:
      ...
      if adat != eredmeny.hexdigest():
        if adat!= "0":
          ...
          print("If the list_of_accounts.txt file has not been modified, this step will be skipped.")
          print(" To ensure the step is always skipped, you can modify the MD5.md5 file to contain the value 0. This will bypass the step, regardless of whether the list_of_accounts.txt file has been changed. ")
          ellenorzes()
...
print("Modified by: https://www.instagram.com/lukka.lekic")
print("\n[{}] >> Starting\n".format(ido))
if hossz > 0:
  fiokok = []
  kerdesek = []
  ...
  while x != ismetles:
    ...
    if (i == 10):
      ...
      ido = datum.strftime("%H:%M:%S")
      print("[{}] >> Break Time (10 Seconds)\n".format(ido))
      time.sleep(2) #You can change this, but if you put shorter time, NGL may ban your IP adress.
      i = 0
      if ismetles != -1:
        if fiok == utolso:
          x += 1
  ...
  while True:
    if (i < 10):
      ...
      if elkuld.status_code == 200:
        ...
        nemsikerult = nemsikerult + 1
        if nemsikerult < 4:
          print("[{} ({}/3)] Oops! Proccess failed! Please check your internet connection. If you think that everything is alright, wait 5 seconds.\n".format(elkuld.status_code,nemsikerult))
          time.sleep(5)
        else:
          datum = datetime.now()
          datumido = datum.strftime("%Y-%m-%d")
